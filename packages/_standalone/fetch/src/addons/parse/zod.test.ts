...
describe('zod', () => {
  ...
  it('should work with ffetch (failure)', async () => {
    ...
    await expect(promise).rejects.toThrowErrorMatchingInlineSnapshot('[Error: Schema validation failed: Invalid input: expected number, received undefined at .a]')
  })
  ...
  it('should work with ffetch safelyParsedJson (pass)', async () => {
    ...
    const res = await ffetch('https://example.com').safelyParsedJson(z.object({ a: z.number() }))
  ...
  it('should work with ffetch safelyParsedJson (failure)', async () => {
    ...
    if (res.issues) {
      ...
      expect(res.issues[0].message).toContain('expected number, received undefined')
    }
  })
})
