...
describe('zod', () => {
  ...
  it('should work with ffetch (failure)', async () => {
    ...
    await expect(promise).rejects.toThrowErrorMatchingInlineSnapshot('[Error: Schema validation failed: a is a required field at .a]')
  })
  ...
  it('should work with ffetch safelyParsedJson (failure)', async () => {
    ...
    if (res.issues) {
      ...
      expect(res.issues[0].message).toContain('a is a required field')
    }
  })
})
